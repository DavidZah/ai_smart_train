...
def print_progress(iteration, total, prefix='', suffix='', decimals=3, bar_length=100):
    ...
    percents = format_str.format(100 * (iteration / (float(total)+1)))  # calculate the % done

    filled_length = int(round(bar_length * iteration / (float(total)+1)))  # calculate the filled bar length
...
if __name__ == '__main__':
    ...
    # test it
    for x in arr:
        video_to_frames(video_path=dir+"\\"+x, frames_dir='test_frames', overwrite=True, every=0.05, chunk_size=100)
    print("\r\ndone")
